$(document).ready(function(e) {
	...
	$('div.proyecto-inicio').on('click','div.contratos i.abrir-subcategorias',function(){
		...
		if($(this).hasClass('fa-plus-square-o')){
			...
			$.getJSON(base_url+'doc/dashboard/historicos',datos,function(json){console.log(json);
				if(json.length>0){
	...
	$('div.proyecto-inicio').on('click','div.contratos .historico',function(){
		...
		if($('span#'+id).attr('valor')==0){
			...
			$.getJSON(base_url+'doc/dashboard/historicos',datos,function(json){
				historico = historico + '<table class="table table-condensed table-bordered table-striped" style="font-size:10px;"><tr><th style="width:5%">Id</th><th style="width:20%">Actividad</th><th>Prioridad</th><th>Descripci&oacute;n</th><th style="width:10%;">Fecha L&iacute;mite</th><th style="width:10%">Estado</th><th style="width:5%">Acci&oacute;n</th><th style="width:5%">Evidencias</th><th style="width:5%">Notificar</th><th style="width:5%">Anotaciones</th></tr>';
				$.each(json,function(x,y){
					historico = historico + '<tr style="background:#F9F9F9" id="programacion'+y.idprogramacion+'"><td class="abrir-programacion" idprogramacion="'+y.idprogramacion+'">P-'+y.idprogramacion+'</td><td>'+y.nombre_actividad+'</td><td>'+y.prioridad.prioridad_nombre+'</td><td>'+y.descripcion_actividad+'</td><td>'+y.fecha.substring(8,10)+'-'+y.fecha.substring(5,7)+'-'+y.fecha.substring(0,4)+'</td><td id="estado">'+y.estado_actividad+'</td><td id="botones" align="center">'+y.botones+'</td><td id="evidencias" align="center" style="color:#555;">'+y.e_doc+' &nbsp; '+y.doc+'</td><td id="notificacion" align="center" style="color:#555;">'+y.mensaje+'</td><td id="anotacion" align="center" style="color:#555;">'+y.a+' &nbsp; <i id="'+y.idprogramacion+'" class="notas fa fa-pencil-square fa-lg"></i></td></tr>';
				});
